...
function RoleCard({
  ...
  return (
    <div className="rounded-[12px] border border-[var(--color-border-hairline)] p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="m-0 text-[15px] font-semibold tracking-[-.01em]">{row.company || "Untitled company"}</p>
          <p className="m-0 mt-[3px] text-[13px] text-[var(--color-muted-fg)]">{row.title || "No title added"}</p>
        </div>
        <span className="mt-[2px] text-[11px] font-semibold text-[var(--color-subtle-fg)]">{roleDateRange(row)}</span>
      </div>

      <div className="mt-3 border-t border-[var(--color-border-hairline)] pt-3">
        <p className="mb-2 text-[11px] font-bold tracking-[.06em] text-[var(--color-subtle-fg)] uppercase">
          ...
          <div className="grid gap-[6px]">
            {entries.map((experience) => (
              <button
                ...
                className="flex items-center justify-between gap-3 rounded-[8px] px-2.5 py-2 text-left text-[13px] transition-colors hover:bg-[var(--color-tag-bg)]"
              >
                <span className="truncate text-[var(--color-body-fg)]">{experience.title}</span>
                <span className="flex-none text-[11px] text-[var(--color-faint-fg)]">{formatDate(experience.date)}</span>
              </button>
            ))}
          </div>
        ) : (
  ...
  if (!hasProfile) {
    return (
      <Card className="structure-card mx-2 p-9 text-center">
        <div className="mx-auto grid h-11 w-11 place-items-center rounded-[12px] bg-[linear-gradient(145deg,#7d67ff,#503bd8)] text-white">
          <Briefcase size={19} />
        </div>
        <h2 className="mt-4 mb-2 text-lg tracking-[-.02em]">Set up your profile</h2>
        <p className="mx-auto mb-5 max-w-[420px] text-[13px] leading-[1.5] text-[var(--color-muted-fg)]">
    ...
    <div className="mx-2 grid gap-5">
      <Card className="structure-card p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="grid h-[52px] w-[52px] flex-none place-items-center rounded-full border-2 border-[#f8ddc8] bg-[#f3b98b] text-[15px] font-semibold text-[#402e27]">
              {initials(profile.name || "?")}
            </div>
            <div>
              <p className="mb-1 text-[11px] font-bold tracking-[.11em] text-[var(--color-muted-fg)]">YOUR PROFILE</p>
              <h1 className="m-0 text-xl tracking-[-.02em]">{profile.name || "Add your name"}</h1>
              <p className="m-0 mt-1 text-[13px] text-[var(--color-muted-fg)]">
      ...
      <Card className="structure-card p-7">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="m-0 text-xs tracking-[.06em] text-[var(--color-subtle-fg)] uppercase">Work history</h3>
          {unlinkedCount > 0 && (
            <span className="flex items-center gap-[5px] text-[11px] text-[var(--color-faint-fg)]">
              <Sparkles size={11} />
