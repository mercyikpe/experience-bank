...
- Optionally, a first-pass guess at the structured fields (Situation/Outcome), pre-filling the existing "Complete this experience" flow instead of leaving it blank. *(Partially in place — starting that flow already seeds Situation from the raw capture and Outcome from the impact field when they haven't been filled in yet; this happens when that step is opened, not at capture-save time. Still not connected to the AI enrichment call above — a candidate for later.)*
...
### 5.6 Voice capture — from browser transcription to backend audio (planned, not yet built)
...
**Open question:** whether to keep `SpeechRecognition` as a fallback for a failed/offline backend call, mirroring the enrichment fallback pattern from 4c, or drop it entirely now that `MediaRecorder` covers more browsers anyway. Leaning toward dropping it — the whole point is fixing Safari/Firefox, and keeping two transcription code paths for marginal benefit adds real complexity.
...
- Polished real-time voice streaming UI — a first pass of record → transcribe → fill field is enough. *(Still true after 5.6's planned move to backend transcription: that's still record → send → transcribe → fill, not live streaming.)*
...
- **Voice transcription: Web Speech API.** Browser-native, free, no backend — consistent with "no server dependency yet." Accepted tradeoff: spottier support in Safari/Firefox than Chrome/Edge; revisit a hosted transcription API if that turns out to matter for the user base. *(Being revisited now that 4c dropped "no backend" as a constraint elsewhere in the product — see new §5.6 for the planned move to backend audio transcription. Not yet built.)*
